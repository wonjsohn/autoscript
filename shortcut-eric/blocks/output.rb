...
class Output
    ...
    def connect_from(source)
        ...
        elsif ["spindle", "waveform", "mixed_input", "triggered_input", "emg", "muscle"].include? block_type
           @input_id += [source.id] unless @input_id.include? source.id
        else
    ...
    def count_wires
      ...
      @input_id.each do |block_type, index|
        ...
        if ["neuron", "spindle"].include? block_type
          @wire_count += 4
        elsif ["synapse", "spike_counter", "triggered_input", "emg", "waveform", "mixed_input", "muscle"].include? block_type
          @wire_count += 2
        end
